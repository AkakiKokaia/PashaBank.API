...
namespace PashaBank.Application.Features.Account.Commands.Bonuses
{
    public class CalculateBonusesAsyncCommandHandler : IRequestHandler<CalculateBonusesAsyncCommand, Response<List<CalculateBonusesAsyncResponseItem>>>
    {
        ...
        {
            var products = _uow.productSalesRepository.GetAllWhere(x =>
                        (!x.WasCalculated)
                        && (request.startDate == null || x.DateOfSale > request.startDate)
                        && (request.endDate == null || x.DateOfSale < request.endDate)
                        && !x.IsDeleted
                    ).ToList();
            await _uow.userService.AccummulatedBonusCalculator(products);

            var allProducts = await _uow.productSalesRepository.GetAllWhere(x =>
                        (request.startDate == null || x.DateOfSale > request.startDate)
                        && (request.endDate == null || x.DateOfSale < request.endDate)
                        && !x.IsDeleted)
                    .ToListAsync();
